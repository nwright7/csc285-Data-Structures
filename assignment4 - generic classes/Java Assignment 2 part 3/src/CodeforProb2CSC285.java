 ...
 * The program currently utilizes a generic class to perform methods upon both objects of type student1 and student2
 * The Program uses generic methods AddItem, sortItems and DeleteItem to add, sort and delete students from an Array List respectively
 * This program contains four classes, class genericClassManager, class Student1, class Student2, and public class CodeforProb2CSC285*/
import java.io.File;
import java.io.FileNotFoundException;
import java.util.ArrayList;
import java.util.Scanner;

/* class genericClassManager contains generic methods and a generic ArrayList so that they may be used on both Objects of Student1 and Student2 */
class genericClassManager<T extends Comparable>{ //a generic class named genericClassManager
    protected ArrayList<T> genericList = new ArrayList<T>(); //creating a generic ArrayList

    public genericClassManager(){} //genericClassManager constructor

    public void AddItem(T item){ //static method AddStudent that adds students to the array list
        genericList.add(item);
    }

    public void sortItems() { //a generic sort method
        T saveItem ,saveItem2;  //saveStudent and saveStudent2 are new objects that will be used to help swap objects in the array
    ...
    public void DeleteItem(T item) { //a general delete method
        genericList.remove(item);
    }

    public int size() { //a method to return size
        return genericList.size();
    }

    public T get(int i) { //a method that returns an object from genericList
 ...
 * the methods within CodeforProb2CSC285 are not used in part 3 of assignment, instead the generic methods
 * from the generic class are used*/
public class CodeforProb2CSC285{
    ...
    public static void main(String[] args) throws FileNotFoundException { //FileNotFoundException needed for reading the input file
        genericClassManager<Student1> student1s = new genericClassManager<Student1>(); //creating an arrayList for object type Student1
        genericClassManager<Student2> student2s = new genericClassManager<Student2>(); //creating an arrayList for object type Student2
        ...
        while(input.hasNext()){ //while loop used to iterate through the input file and assign variables their values
            ...
            Student1 workStu1 = new Student1(test, Sid, name); //creating a new Student1 object
            student1s.AddItem(workStu1); //storing object in student1s
        }//end of while loop
        System.out.println("----------------------------------------------------------------------------------------------" + "\n" + "Part 1 Student Objects using a Generic Class"
        + "\n" + "----------------------------------------------------------------------------------------------");
        ...
        for(int i =0; i < student1s.size(); i++){ //for loop is used to iterate through the ArrayList and print out each object
            System.out.println(student1s.get(i));
        }

        for(int i=0; i<student1s.size(); i++){ //a for loop that is finding the object within our array that we want to remove and removing it
            if(student1s.get(i).id.equals("42P4")){
                student1s.DeleteItem(student1s.get(i));
            }
            if(student1s.get(i).id.equals("45A3")){
                student1s.DeleteItem(student1s.get(i));
            }
        }
        ...
        student1s.AddItem(new Student1(new int[]{80,75,98},"67T4","Clouse,B"));  //adding new students to the array list
        student1s.AddItem(new Student1(new int[]{75,78,72},"45P5","Garrison,J"));
        student1s.AddItem(new Student1(new int[]{85,95,99},"89P0","Singer,A"));
        ...
        input = new Scanner(new File("Input2.txt")); //creating the scanner to read through the input file
        while(input.hasNext()){ //while loop used to iterate through the input file and assign variables their values
            int[] test = new int[3]; //creating variables that will be used to temporarily store the values being read in from input text before they are put into an object
            String Sid = input.next();
            String name = input.next();
            ...
            student2s.AddItem(workStu2); //storing object in student2s
        }//end of while loop

        System.out.println("\n" + "----------------------------------------------------------------------------------------------" + "\n" +
                "Part 2 Student Objects using a Generic Class"
                + "\n" + "----------------------------------------------------------------------------------------------");
        System.out.println("List the objects from the class ArrayList including the % score and the grades.");
        ...
        for(int i =0; i < student2s.size(); i++){ //for loop is used to iterate through the ArrayList and print out each object
            System.out.println(student2s.get(i));
        }

        for(int i=0; i<student2s.size(); i++){ //a for loop that is removing the object from our array list with the matching student id
            if(student2s.get(i).id.equals("42P4")){
                student2s.DeleteItem(student2s.get(i));
            }
            if(student2s.get(i).id.equals("45A3")){
                student2s.DeleteItem(student2s.get(i));
            }
        }
        ...
        student2s.AddItem(new Student2(new int[]{80,75,98},"67T4","Clouse,B", 102, (float)3.65));  //adding new students to the array list
        student2s.AddItem(new Student2(new int[]{75,78,72},"45P5","Garrison,J", 39, (float)1.85));
        student2s.AddItem(new Student2(new int[]{85,95,99},"89P0","Singer,A", 130, (float)3.87));

        student2s.sortItems(); //calling upon the sortItems method to sort the student objects within the array list
